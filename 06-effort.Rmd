...
Eric Taylor [-@taylor2015new] surveyed the evaluation of new education technology and concluded that education technologies are generally pedagogically ineffective in raising test scores. There are two competing hypotheses in explaining this observation: 
...
The introduction argues that the evaluation of ITS development should distinguish between effectiveness and efficacy, but what is the ideal implementation condition that allows efficacy to be evaluated? Ericsson et al [-@ericsson1993role;-@ericsson2016peak] and Duckworth et al[-@duckworth2007grit] argues that passion and grit are essential for practice to be effective. Following this school of thought, this chapter regards the ideal condition for implementation as learners exert their best effort no matter how challenging or boring the learning task is. 
...
\begin{equation}
ATE = [E(Y_{t=1}|D=T)-E(Y_{t=1}|D=C)]-[E(Y_{t=0}|D=T)-E(Y_{t=0}|D=C)] \label{ate}
\end{equation}


The DID estimator accounts for common growth from pre-test to post-test period ($\beta_t$). It also controls for potential difference in the correct rate between the treatment and control groups in the pre-test period ($\beta_D$). Let $\beta_0$ be the average correct rate for the control group at the pre-test period. Let $\gamma$ denote the ATE. The four components in equation \ref{eq:ate} can be expressed as:
$$
\begin{aligned}
E(Y|D=C,t=0) &= \beta_0\\
...
&= (1-\pi)(1-\ell_0)(1-\ell_D)(c_1^0-c_0^{1,0})+((1-\pi)[(1-\ell_0)\ell_D+\ell_0])(c_1^1-c_0^{1,0})\\
&+(1-\pi)(c_1^1-c_0^{1,1})
\end{aligned}
$$
...
&= (1-\pi)(1-\ell_0e^0_0)(1-\ell_De_D)(c_1^0e^0_1-c_0^{1,0}e^0_0)\\
&\quad+((1-\pi)[(1-\ell_0e^0_0)\ell_De^0_D+\ell_0e^0_0])(c_1^1e^1_1-c_0^{1,0}e^0_0)+(1-\pi)(c_1^1e^1_1-c_0^{1,1}e^0_0)
\end{aligned}
$$
...
Theorem 5.3 essentially says the DID estimator correctly recovers the rank order only when the effort choices of learners without mastery are the same for the treatment pedagogy and the control pedagogy. It implies that the DID estimator does not preserve the rank order of efficacies when the effort choices differ by practice questions, which is more realistic in a low stake learning environment.
...
kable(
  ...
  caption = '\\label{tab:s_did}Simulation Result of the DID estimator'
)
```

In contrast, Table \ref{tab:s_ltp} shows that the LTP estimator always recovers the correct rank order whether the data generating process has the effort choice or not. 
...
kable(
  ...
  booktabs = TRUE, align='c',format='pandoc',
  caption = '\\label{tab:s_ltp}Simulation Result of the LTP estimator'
)
...
The experiment is carried out in a paid self-learning product offered by a Chinese online learning service provider. The product is used after school, rather than in the classroom. The product is framed as a role-playing game where a learner clears a level to claim some reward by succeeding in practice questions. The screen shots of the selection, the practice and the completion of a level are shown in Figure \ref{fig:interface_afenti}.

```{r,echo=FALSE,message=FALSE,warning=FALSE, fig.cap = "\\label{fig:interface_afenti}Stages of a Level in the Learning Game", fig.align='center'}
...
Table \ref{tab:ans_wrong} shows a breakdown of the answer classification. The classification is discussed in details in Appendix C.
...
kable(
  ...
  col.names=c('Group','Task','No-Effort(%)','S.E.','Honest Error (%)','S.E.', 'Correct(%)','S.E.'),
  booktabs = TRUE, align='c',format='pandoc',
  caption = '\\label{tab:ans_wrong}Answer Composition - All Wrong'
)
```
...
In contrast, answers with actual effort input do not demonstrate the same shift. Figure \ref{fig:dist_time_effort} shows the response time distribution when submitting a valid answer. A honest error or a correct answer is taxing on one's mind and takes time, while a no-effort error made on the fly. The comparison of response time distribution between effort exertion and the lack of it further substantiates the classification.


```{r,echo=FALSE,message=FALSE,warning=FALSE, fig.cap = "\\label{fig:dist_time_effort}Distribution of Time Spent on Item With Effort by Answer Classification", fig.align='center',out.height='8cm',out.width='8cm'}
...
kable(effort_rate_stat, 
      ...
      booktabs = TRUE, align='c', format='pandoc',
      caption = '\\label{tab:erc_all}Effort Rate Choices - All'
      ) 
```
...
kable(effort_rate_stat_placebo, 
      booktabs = TRUE,
      col.names = c('Period','Group','Mean Effort Rate(Y=0)','S.E.','Mean Effort Rate(Y=1)','S.E.'),
      align='c',format='pandoc',
      caption = '\\label{tab:erc_res}Effort Rate Choice - By Response in the Pre-test'
      ) 
```
...
kable(
  ...
  col.names=c('Model','Est. Rel Effective','Est. Rel Efficacy','95% CI(L)','95% CI(H)','Treatment Win'),
  align='c',format='pandoc',
  caption = '\\label{tab:el_aggr}Estimated Relative Effectiveness and Relative Efficacy in Aggregate Score'
)
...
kable(
  param_data, booktabs = TRUE,
  col.names=c('Knowledge','Model','Est. Rel  Effect','Est. Rel  Efficacy','95% CI(L)','95% CI(H)'),
  align='c',format='pandoc',
  caption = '\\label{tab:el_comp}Estimated Relative Effectiveness and Relative Efficacy by Knowledge Components'
)
