...
The ambition of this chapter is much smaller. If the learner fails to correctly solve a problem, the learning recommendation seeks to supply him/her with a set of remedial material so that he/she can solve another problem with identical latent knowledge requirement and skill requirement. This goal is rooted in the literature of deliberate practice[@ericsson1993role;@ericsson2016peak], which seeks to explain how experts are trained in a narrow field. Deliberate practice emphasizes self-motivated repetitive exercises with immediate informative feedback that aims to improve the result. From the pedagocial perspective, the difficult yet critical point is to provide immediate yet informative feedback that incorporates learners' pre-existing knowledge and current flaw. Given same distribution motivation and talent, the difference in learning outcome is mainly driven by the difference in feedback quality. Therefore, in the framework of deliberate practice, pedagogical efficacy can be defined as the improvement of the task performance.

For practical reasons discussed in the next section, pedagogical efficacy is defined as delta over a naive repetition strategy rather than the absolute task improvement (or learning gain). An accurate measurement of the latter requires stronger assumption, which is unlikely to be met in practice. This is a slight difference from Khan Academy's learning gain metrics[@faus2015systems].

## Research Design

Although massive online experiment(MOE) in education is not new ideas[@williams_mooclet_2014], and many experiments have been carried out in the industry (most noticeably by Khan Academy and Duolingo), with few exceptions[@paunesku2015mind], the idea is yet to catch fire in the academia. By the highest academic standard, MOE suffers from two critical weakness: selection bias induced by differential user attrition and measurement error induced by slacking. This section details the design response to mitigate these two biases.
...
Acknowledging that attrition is common in field experiments and only differential attrition matters, What works clearinghouse evidence standard 3.0[@clearinghouse2008wwc] provides a guideline on what user attrition profile is acceptable. (**better explanation needed here.**)
...
The experiment primarily relies on the assumption that the distribution of slacking is identical with random assignment. Although the point estimation of absolute instructional efficacy is biased, the relative ranking of different pedagogical methods is consistently identified. However, since the experiment has access to the student's response time and answer, a direct identification of the measurement error is also attempted. A detailed identification procedure is described in appendix II.
...
Retain those did not give up on the original question and the training question, as to ensure they are at least somewhat exposed to the learning task. None of the effects is statistically significant. From the magnitude of the point estimation, video scaffolding is more effective for the first assessment but not necessary for the second assessment. That said, the time period effect is surprisingly strong.
