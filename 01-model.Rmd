...
Furthermore, assume that if the learner does not exert effort, the response is always false. 
...
$$
h^k_{t,j}=P(H_t=1|H_{t-1}=0,X_t=k,A_t=j)
$$

**Assumption 10**: Stop decision is independent of item characteristics

**Assumption 11**: The duration dependence of the hazard rate is proportional


Under assumption 10 and 11, the hazard rate can be expressed as:

$$
h_t^k = \lambda_k e^{\beta_k t}
$$

The differential hazard rate can be mechanical. For example, if the learner is forced to stop after X failures, the hazard rate rises as the number of failures accumulates. Since lower mastery learner are more prone to error, they are more prone to stop. However, the latent state can influence the hazard rate by the active choice of the learner. The author suspects two reasons for attrition, boredom and frustration. If the learner has already mastered the knowledge point, further practices produce no learning gain and they are bored. If the learner hasn't mastered the knowledge point after a few exercises, they are frustrated. This may cause the differential hazard rate based on response observed in data. Furthermore, the author speculates that frustration hurts the learner's engagement more than the boredom.
