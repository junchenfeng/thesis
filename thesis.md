...
- Practice as Instruction -> Optiomal Recommendation
  ...
  + What matters is how the item helps the learner to elevate the latent mastery
  + What we usually observe is how difficult the item given the current latent mastery
  + The relationship between the item efficacy and item and item difficulty is unclear
  + Folk wisdom is inverse U
...
========================================================
- In Theory
  + Reveal information about the user if the decision depends on the state
  + Capture some noncognitive skills, like grit
...
**Assumption 10**: Effort choice is independent, conditioning on the latent ability and item
...
- It implies that the learner does not learn about the cost of effort
...
- The hazard rate varies with the latent state
  + Not depends on response. An error due to slip and an error due to lack of understanding have different implications
...
The General Model as A Hidden Markov Process
========================================================
id: hmm2
...
========================================================
  ...
  + $\mathbf{X}^i$: The latent state sequence, $X^i_1,\dots,X^i_{T_i}$
  + $\mathbf{Y}^i$: The answer sequence, $Y^i_1,\dots,Y^i_{T_i}$
  + $\mathbf{A}^i$: The item sequence,   $A^i(1),\dots,A^i(T_i)$
  + $\mathbf{E}^i$: The effort sequence, $E^i_1,\dots,E^i_{T_i}$
  + $\mathbf{H}^i$: The stop sequence,   $H^i_1,\dots,H^i_{T_i}$


The Sufficient Statistics
========================================================
**Identifcation Assumption 1**: Learners are homogeneous and independent

Let $G(\mathbf{Y},\mathbf{A}, \mathbf{E},\mathbf{H})$ maps the observed variable to a one dimension set $\Omega$ with size $Z$.

Let $G^{-1}(z)$ denote the $z$ element of mapping set.
...
$$
It is easy to see that $\{n1,\dots,n_Z\}$ are sufficient statistics where $n_z=\sum_{i=1}^N1(G(\mathbf{Y}^i,\mathbf{A}^i,\mathbf{E}^i,\mathbf{H}^i)=z)$

The Sufficient Statistics (Cont)
========================================================
- Example:
  + Response only, binary $Y_t$, $t$=2, $j=1$
  + $\Omega=\{\{Y_1=1,Y_2=1\},\{Y_1=0,Y_2=1\},\{Y_1=1,Y_2=0\},\{Y_1=0,Y_2=0\}\}$
  + $G(\{Y_1=1,Y_2=1\})=1$, $G^{-1}(1) = \{Y_1=1,Y_2=1\}$


The Augmented Likelihood(1)
========================================================
Let $G^{-1}(z) = \mathbf{y}, \mathbf{a}, \mathbf{e}, \mathbf{h}$ and $T_z$ be the length of the practice sequence.

$$
P(G^{-1}(z)) = \sum_{X_1}\dots\sum_{X_{T_z}}P(\mathbf{y}, \mathbf{a}, \mathbf{e}, \mathbf{h}, \mathbf{X})
$$

Assume $\mathbf{X}$ is known(augmented). Assume that item sequences are exogeneously determined, Thus $\mathbf{a}$ is independent of all other variables.

$$
\begin{aligned}
P(G^{-1}(z)) &= P(\mathbf{y},\mathbf{e}, \mathbf{h}, \mathbf{X}| \mathbf{a})\\
...
$$
\begin{aligned}
P(\mathbf{y}|\mathbf{X},\mathbf{e},\mathbf{a}) &= \prod_{t=1}^{T_z} \prod_{k=1}^K\prod_{j=1}^J[ (c_j^{0,k})^{1(A(t)=j,y_t=0,x_t=k,e_t=1)}1^{1(A(t)=j,y_t=0,x_t=k,e_t=0}\\
&\prod_{m=1}^{M-1} (c_j^{m,k})^{1(A(t)=j,y_t=m,x_t=k,e_t=1)}0^{1(A(t)=j,y_t=m,x_t=k,e_t=0}]
\end{aligned}
$$
...
$$
\begin{aligned}
P(\mathbf{h}|\mathbf{X}) &= \prod_{t=1}^{T_z}\prod_{k=1}^Kh(t,k)^{1(H_t=1,X_t=k)}(1-h(t,k))^{1(H_t=0,X_t=k)}\\
&= \prod_{t=1}^{T_z}\prod_{k=1}^K[\lambda_ke^{\beta_kt}]^{1(H_t=1,X_t=k)}(1-\lambda_ke^{\beta_kt})^{1(H_t=0,X_t=k)}
\end{aligned}
$$
...
$$
\begin{aligned}
&P(\mathbf{X},\mathbf{e}|\mathbf{a})\\
&= P(X_1)P(E_1)\prod_{t=2}^{T_z}P(E_t|X_{t-1})P(X_t|X_{t-1},E_{t})\\
&= \prod_{k=1}^K(\pi^k)^{1(X_1=k)}\prod_{j=1}^J(e_j^k)^{1(A(t)=j,X_1=k,E_1=1)}(1-e_j^k)^{1(A(t)=j,X_1=k,E_1=0)}\\
& \prod_{t=2}^{T_z}\prod_{j=1}^J\{[(e_j^k)^{1(A(t)=j,X_{t-1}=k,E_t=1)}(1-e_j^k)^{A(t)=j,1(X_{t-1}=k,E_t=0)}]\\
&[1^{1(X_{t-1}=K,X_t=K)}\prod_{k=1}^{K-1}(\ell^k_j)^{1(A(t)=j,X_{t-1}=k,X_t=k+1,E_t=1)}(1-\ell^k_j)^{1(A(t)=j,X_{t-1}=k,X_t=k,E_t=1)}\\
...
- The state density is non-degenerate
  + **Identifcation Assumption 3** $\pi^k \neq 0$

Identification (2)
========================================================

- For MLE:
  + Unique identification matters

- For MCMC:
  + Only requires proper posterior distribution
  + MAP point estimator is meaningless
  + Convergence is problematic

Identification for MLE
========================================================
- Recall sufficient statistics $\{n_1,\dots,n_Z\}$. It implies Z moment conditions.
- Let the size of the parameter set $\Theta$ to be W.
- Necessary Condition: $Z\geq W$
- Sufficient Condition: Nonlinear system makes it difficult to prove

- Example($j=1$, $t=2$,$E_t=1$,$H_t=0$):

  + $X_t$,$Y_t$ binary, Z=4, W=4

  + $X_t$ tertiary, $Y_t$ binary, Z=4, W=6

  + $X_t$,$Y_t$ tertiary,  Z=9, W=11

**Identifcation Assumption 4**:$c^{0,2}_j=c^{2,0}_j=0$
...
- Why not EM:
  ...
  + Hard to incorporate user heterogeneity(Future)
...
========================================================
- Brute Force
  + $P(X_1|\mathbf{Y},\mathbf{E},\mathbf{H},\mathbf{A})$
  + $P(X_t|X_{t-1},\mathbf{Y},\mathbf{E},\mathbf{H},\mathbf{A})$
  + Sample forward

- Forward recursion, backward sampling[Scott, 2002]
  + $\tilde{\pi}^k_t=P(X_t=k|\mathbf{Y}_{1,t},\mathbf{E}_{1,t},\mathbf{A}_{1,t} ,H_t, \Theta)$
  + $P_{t}(i,j) = P(X_t=i,X_{t+1}=j|\mathbf{Y}_{1,t+1},\mathbf{E}_{1,t+1},\mathbf{A}_{1,t+1},H_{t+1},\Theta)$
  + reduce the computation cost

Data Augmentation: FRBS recursion
========================================================
...
$$
\begin{aligned}
E(\hat{\ell^k}) &= \frac{\sum_{t=1}^TP(X_t=k+1|X_{t-1}=k,H_t=0|\mathbf{Y},\mathbf{E})P(H_t=0,X_t=k+1,X_{t-1}=k|\mathbf{Y},\mathbf{E})}{\sum_{t=1}^TP(H_t=0,X_{t-1}=k,\mathbf{Y},\mathbf{E})}\\
&= \ell\times \frac{\sum_{t=1}^TP(H_t=0|X_t=k+1,X_{t-1}=k|\mathbf{Y},\mathbf{E})P(X_{t-1}=k|\mathbf{Y},\mathbf{E})}{\sum_{t=1}^TP(H_t=0|X_{t-1}=k,\mathbf{Y},\mathbf{E})P(X_{t-1}=k|\mathbf{Y},\mathbf{E})}
\end{aligned}
$$
- Unbiase if and only if $P(H_t|X_t=k+1,X_{t-1}=k)=P(H_t|X_{t-1}=k)$, which means the hazard rate is independent of latent state
...
- Sequential Order: Grade 1
  + 1,4,2. Which is the 2nd largest number?
