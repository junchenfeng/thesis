...
- Practice as Instruction -> **Optiomal Recommendation**
  ...
  + What matters is how the item helps the learner to elevate future mastery
  + What we usually observe is how difficult the item given the current mastery
  + The relationship between the item efficacy and item difficulty is unclear
...
- The hazard rate varies with the latent state
  + Not depends on response.
  + An error due to slip and an error due to lack of mastery should have different implications
...
-It is easy to see that $\{n1,\dots,n_Z\}$ are sufficient statistics
...
========================================================
Assume that item sequences are exogeneously determined, Thus $\mathbf{a}$ is independent of all other variables.

Let $G^{-1}(z) = (\mathbf{y}, \mathbf{a}, \mathbf{e}, \mathbf{h})$ and $T_z$ be the length of the practice sequence.

$$
P(G^{-1}(z)) = \sum_{X_1}\dots\sum_{X_{T_z}}P(\mathbf{Y}=\mathbf{y},  \mathbf{E}=\mathbf{e}, \mathbf{H}=\mathbf{h}, \mathbf{X}|\mathbf{a})
$$

For a particular state augmentation $\mathbf{X}$

$$
\begin{aligned}
P(\mathbf{Y}=\mathbf{y},  \mathbf{E}=\mathbf{e}, \mathbf{H}=\mathbf{h}, \mathbf{X}|\mathbf{a}) &= P(\mathbf{y},\mathbf{e}, \mathbf{h}, \mathbf{X}| \mathbf{a})\\
&= P(\mathbf{Y}=\mathbf{y}|\mathbf{X},\mathbf{a},\mathbf{E}=\mathbf{e})P(\mathbf{H}=\mathbf{h}|\mathbf{X})P(\mathbf{E}=\mathbf{e},\mathbf{X}|\mathbf{a})
\end{aligned}
$$
...
$$
\begin{aligned}
P(\mathbf{Y}=\mathbf{y}|\mathbf{X},\mathbf{E}=\mathbf{e},\mathbf{a}) &= \prod_{t=1}^{T_z} \prod_{k=1}^{M_x}\prod_{j=1}^J[ (c_j^{0,k})^{1(A(t)=j,y_t=0,x_t=k,e_t=1)}1^{1(A(t)=j,y_t=0,x_t=k,e_t=0}\\
...
$$
\begin{aligned}
P(\mathbf{H}=\mathbf{h}|\mathbf{X}) &= \prod_{t=1}^{T_z}\prod_{k=1}^{M_x}h(t,k)^{1(H_t=1,X_t=k)}(1-h(t,k))^{1(H_t=0,X_t=k)}\\
&= \prod_{t=1}^{T_z}\prod_{k=1}^{M_x}[\lambda_ke^{\beta_kt}]^{1(H_t=1,X_t=k)}(1-\lambda_ke^{\beta_kt})^{1(H_t=0,X_t=k)}
\end{aligned}
$$
...
$$
\begin{aligned}
&P(\mathbf{X},\mathbf{E}=\mathbf{e}|\mathbf{a})\\
&= P(X_1)P(E_1|X_1)\prod_{t=2}^{T_z}P(E_t|X_{t-1})P(X_t|X_{t-1},E_{t})\\
&= \prod_{k=1}^{M_x}(\pi^k)^{1(X_1=k)}\prod_{j=1}^J(e_j^k)^{1(A(t)=j,X_1=k,E_1=1)}(1-e_j^k)^{1(A(t)=j,X_1=k,E_1=0)}\\
& \prod_{t=2}^{T_z}\prod_{j=1}^J\{[(e_j^k)^{1(A(t)=j,X_{t-1}=k,E_t=1)}(1-e_j^k)^{1(A(t)=j,X_{t-1}=k,E_t=0)}]\\
...
- Sufficient Condition: difficult for nonlinear system
